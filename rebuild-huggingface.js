...
/**
 * 🔄 سكريبت إعادة بناء خادم Hugging Face
 * 
 * هذا السكريبت يضمن أن الإصلاحات الجديدة يتم تطبيقها على الخادم
 */

const fs = require('fs');
const path = require('path');

class HuggingFaceRebuilder {
  constructor() {
    this.sourceDir = 'faster-whisper-api';
    this.targetDir = 'huggingface_deploy';
    this.filesToUpdate = [
      'app.py',
      'requirements.txt',
      'README.md'
  ...
  async rebuild() {
    console.log('🔄 بدء إعادة بناء خادم Hugging Face...');
    console.log('');
    ...
    try {
      ...
      // إنشاء ملف .gitignore للخادم
      await this.createGitignore();
      
      console.log('✅ تم إعادة بناء الخادم بنجاح!');
      console.log('');
      ...
      console.log('  ✅ إصلاح معالجة الملفات الصوتية');
      console.log('');
      ...
      console.log('');
      console.log('⚠️ ملاحظة: قد يستغرق إعادة البناء 2-5 دقائق');
      console.log('💡 تحقق من حالة البناء في Hugging Face Spaces');
  ...
  async createUpdatedReadme() {
    const readmeContent = `# 🎤 Faster Whisper API - Rebuilt Version
...
// تشغيل السكريبت
async function main() {
  const rebuilder = new HuggingFaceRebuilder();
...
module.exports = HuggingFaceRebuilder; 