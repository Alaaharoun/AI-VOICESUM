...
export class TranscriptionEngineService {
  ...
  async getCurrentEngine(): Promise<TranscriptionEngine> {
    try {
      ...
      if (error) {
        ...
        // في حالة الخطأ، نستخدم Hugging Face كافتراضي
      ...
      // إذا لم تكن هناك إعدادات، نستخدم Hugging Face كافتراضي
      return 'huggingface'; // Default to Hugging Face
    } catch (error) {
  ...
  async getWebSocketURL(): Promise<string> {
    ...
    if (engine === 'huggingface') {
      // For Hugging Face, we don't use WebSocket - we use HTTP API
      // But we can return a placeholder or throw an error
      throw new Error('Hugging Face engine does not use WebSocket connections');
    } else {
      // Default to Azure/Render server
      return 'wss://ai-voicesum.onrender.com/ws';
    }
  }

  /**
   * Get the appropriate server URL for the current engine
   */
  async getServerURL(): Promise<string> {
    const engine = await this.getCurrentEngine();
    
    if (engine === 'huggingface') {
      return 'https://alaaharoun-faster-whisper-api.hf.space';
    } else {
      // Default to Azure/Render server
      return 'https://ai-voicesum.onrender.com';
    }
  }
