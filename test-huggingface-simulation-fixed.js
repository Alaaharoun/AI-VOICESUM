...
console.log('🎭 Hugging Face Faster Whisper Simulation Test (Fixed Version)');
...
async function simulateRealTimeTranscription() {
  ...
  try {
    ...
    for (let i = 0; i < audioChunks.length; i++) {
      ...
      if (response.ok) {
        ...
        if (data.success && data.text) {
          ...
          console.log('⚠️ No transcription text returned (empty audio)');
        }
      } else {
    ...
    autoFormData.append('file', autoAudio, {
      filename: 'auto_detect.wav',
      contentType: 'audio/wav'
    });
    ...
    translateFormData.append('file', translateAudio, {
...
async function runSimulation() {
  console.log('🚀 Starting Hugging Face Simulation (Fixed Version)...\n');
