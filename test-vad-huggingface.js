...
class MockSpeechService {
  ...
  static async transcribeAudioRealTime(audioBlob, targetLanguage, sourceLanguage, useLiveTranslationServer, useVAD = false) {
    try {
...
// Test function
async function testVADWithHuggingFace() {
  console.log('🧪 Starting VAD test with Hugging Face...\n');
  ...
  try {
    ...
    console.log('📝 Test 1: Transcription without VAD');
    ...
    // Test 3: Real-time transcription with VAD
    console.log('\n📝 Test 3: Real-time transcription with VAD');
    console.log('=' .repeat(50));
    
    const realTimeResult = await MockSpeechService.transcribeAudioRealTime(mockAudioBlob, 'ar', 'en', false, true);
    console.log('✅ Real-time transcription with VAD result:', realTimeResult);
...
// Run the test
testVADWithHuggingFace().catch(console.error); 